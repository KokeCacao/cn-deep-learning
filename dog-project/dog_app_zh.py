...
# 
# __回答:__
# It does not make sence. Because the machine can absolutely be better than human in detecting faces. We can use image augmentation to add random noise in the picture, making the image vague to solve this problem.
# 
...
for human in humans:# (224, 244, 3)
    human = np.expand_dims(human, axis=0) # I cannot use directory because I don't know where udacity stores the files
    if dog_detector(human): human_count=human_count+1
for dog in dogs:
...
# - BatchNormalization: Normalize the data so that the model can fit the data better (the intuition is that the normalization create more space between datapoints so that the classification can be better. Although normalizations don't change the result theriotically, they do help in practice.)
# - Activation(relu): relu ensures that the zero-gradient problems don't exist. It is also zero centered to avoid zig-zag into global minimum.
...
model.add(Conv2D(16, (3, 3), input_shape=(224,224,3),activation=None, padding="valid"))
model.add(MaxPooling2D((2, 2)))
model.add(BatchNormalization())
model.add(Activation('relu'))
model.add(Conv2D(32, (3, 3), activation=None, padding="valid"))
model.add(MaxPooling2D((2, 2)))
model.add(BatchNormalization())
model.add(Activation('relu'))
...
# - Firstly, it cannot handle adversarial inputs since our model only trained without augmentation. We can augmente data (flip, rotate, and add random noise)
# - Secondly, the model may think cats are dogs because there are only dogs in our training sample. We can add more categories.
# - Thirdly, the prediction need a lot of time to generate on giant network and has a potential of overfitting. We can add weight decade.
